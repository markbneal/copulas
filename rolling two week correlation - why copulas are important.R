...
# https://eranraviv.com/live-rolling-correlation-plot/
# see also
...
fun = function(){
  ...
  dat0 = (getSymbols(symetf[1], src="yahoo", from=start, to=end, auto.assign = F, warnings = FALSE,symbol.lookup = F))
  n = NROW(dat0)  ; l = length(symetf)
  dat = array(dim = c(n,NCOL(dat0),l)) ; ret = matrix(nrow = n, ncol = l) 
  for (i in 1:l){
    dat0 = (getSymbols(symetf[i], src="yahoo", from=start, to=end, auto.assign = F,warnings = FALSE,symbol.lookup = F))
    dat[1:n,,i] = dat0 
